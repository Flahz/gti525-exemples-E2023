Promise.most = function (proms) {
    return new Promise((resolve, reject) => {
        ...
        let countReject = 0;
        if (proms.length === 0) {
            // Si aucune promesse n'est fournie, on considère que c'est une "résolution"
            resolve("Aucune promesse fournie.");
            return;
        }
        // Utiliser Promise.allSettled pour obtenir tous les états
        Promise.allSettled(proms).then((results) => {
            ...
            // Vérifier si au moins 50% des promesses sont résolues
...
Promise.most(proms).then(function () {
    console.log("La majorité des promesses ont été résolues 1");
}).catch(function (err) {
    // Cette ligne sera exécutée
    console.log("La majorité des promesses ont été rejetées 1");
});
...
Promise.most(proms).then(function () {
    // Cette ligne sera exécutée
    console.log("La majorité des promesses ont été résolues 2");
}).catch(function (err) {
    console.log("La majorité des promesses ont été rejetées 2");
});